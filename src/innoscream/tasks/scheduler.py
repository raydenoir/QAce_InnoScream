...
async def post_weekly_stress_graph():
    """Post the weekly stress graph to the main channel."""
    today = date.today()
    ...
    if chart_image_url:
        screams_count = sum(data)

        avg_screams = screams_count / 7.0 if screams_count > 0 else 0.0

        previous_sunday = previous_monday + timedelta(days=6)

        caption_parts = [
            (
                f"📊 Weekly Stress Report ({previous_monday:%d %b} "
                f"- {previous_sunday:%d %b %Y}) 📊"
            ),
            "",
            "Screams per day:"
        ]
        ...
        caption_parts.append(f"Total this week: {screams_count}")
        caption_parts.append(
            f"Weekly Average: {avg_screams:.1f} screams/day"
        )
